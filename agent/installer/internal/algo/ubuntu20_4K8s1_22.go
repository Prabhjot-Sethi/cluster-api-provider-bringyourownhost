...
func (u *Ubuntu20_4K8s1_22) containerdStep(bki *BaseK8sInstaller) Step {
	...
	doCmd += " && if [ ! -e /etc/containerd/config.toml ]; then mkdir -p /etc/containerd && "
	doCmd += GetConfigTomlEchoString()
	doCmd += " > /etc/containerd/config.toml && chmod 755 /etc/containerd/ && chmod 644 /etc/containerd/config.toml; fi"
